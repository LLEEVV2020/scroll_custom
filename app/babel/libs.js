...
function creatuRerunner(scroll) {
  ...
  var table_w = table.width(); // вычисляем сколько процентов составляет "бегунок" от общего размера видимого окна
...
function makeScroll(scroll, x) {
  ...
  var runnerWidth = parseInt(runner.css("width")); // ставим левый отступ

  var margin_left; // сдвигаем влево
...
function makeScroll2(scroll, x) {
  ...
  console.log(Math.abs(savex - x)); // сдвигаем влево
